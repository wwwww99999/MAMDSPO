...
class Agent:
    ...
    def update(self, data, global_states):
        ...
        td_r_target = torch.tensor(data['td_r_targets'], dtype=torch.float).to(self.device)
        td_c_target = torch.tensor(data['td_c_targets'], dtype=torch.float).to(self.device)
        global_states = torch.tensor(global_states, dtype=torch.float).to(self.device)
        ...
        dataset_3 = TensorDataset(global_states, td_r_target, td_c_target)
        loader_3 = DataLoader(dataset=dataset_3, batch_size=self.minibatch, shuffle=True)

        for i in range(self.epoch):
            for _, (global_states_b, td_r_target_b, td_c_target_b) in enumerate(loader_3):

                # Update reward critic
                r_value_loss = torch.mean(F.mse_loss(self.r_value(global_states_b), td_r_target_b))
                ...
                # Update cost critic
                c_value_loss = torch.mean(F.mse_loss(self.c_value(global_states_b), td_c_target_b))
